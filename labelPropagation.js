...
// label propagation algorithms is the fol- lowing: the algorithms execute in rounds, and at the begin- ning of each
// round every node has a label representing the cluster that the node currently belongs (at the beginning, ev- ery node
...
jLabelPropagation = function () { // A function expression can be stored in a variable. After a function expression has been
    ...
    function __modularity(status) { // Only with graph.status, it is possible to calculate the respective modularity.
        ...
        nodes.forEach(function (node) {
            ...
            let gdegree = status.gdegrees[node] || 0; // CHECK THIS!!! I think it should be 0.
    ...
    let core = function () {
        ...
        return partition_at_level(dendogram, dendogram.length - 1);     // Final output of the Label Propagation algorithm.
    };
